...
| app-winui | `src/app-winui/*.cpp + *.xaml` | ✅ WinUI 3 壳、只读 EditorRenderFrame、原生编辑面、独立滚动、TSF/IME、键盘、指针、异步文档 I/O 与侧边栏导航控制器生命周期、MathJax、Mermaid、Tree-sitter |

### 缺失/待补的 core 模块

- ~~`scheduler.ixx`~~：✅ 已加。
- ~~`caret.ixx`~~：✅ 已加（renderer 用）。
- ~~扩展注册表~~：✅ `extension.ixx` 提供去重、查询、版本与 dialect 能力组合；Editor 的全量和增量解析共享配置后的 dialect。
...
```
键盘/鼠标/IME → platform input → EditorInputEvent → Command → SemanticEditPlan → Transaction
→ TextBuffer(revision++) → parse → MarkdownDocument+SourceMap+Diagnostics
→ build_render_model → RenderModel → EditorRenderFrame → DocumentLayoutPlan → block measure/paint → LayoutTree
→ Direct2D/DirectWrite 自绘
```

每层都带 revision；旧 revision 结果丢弃；undo/redo 也要 revision++（绝不重置）。
TextBuffer 没有从 Editor 暴露可写引用，WinUI 控制器只能读取 EditorSession 的不可变视图；
除打开文档外，所有源码变化必须由 Command 生成语义 Transaction 后进入 buffer。
...
cmake --build build --target elmd_tests     # core + 277 tests
