...
`EditorDocument` 的统一块树，以及各可编辑内容节点持有的 `InlineDocument.source` 与 lossless inline CST，是文档解析完成后唯一可信的编辑状态。

```text
Markdown -> Parser -> 统一块树 + 块内 InlineDocument
块内 TextEdit / 块级树操作 -> EditorDocument
EditorDocument -> Renderer -> 界面
EditorDocument -> Serializer -> Markdown
```

完整 Markdown 文本、RenderModel、LayoutTree 和屏幕坐标均为派生数据。块内 `InlineDocument.source` 是该内容节点逐字符无损的权威字符内容；文档不保存全文 SourceMap。渲染与命中测试使用 `TextPosition`/`TextSpan`，其中块内位置只有 `container_id + source_offset + affinity` 一套坐标。

常规编辑交互不得依赖扫描 Markdown 行前缀、统计缩进或临时拼接 Markdown 字符串。Markdown 语法识别属于 Parser，Markdown 文本生成属于 Serializer；键盘交互必须落到块内源码编辑或块树结构命令之一。
...
编辑位置应基于内容容器的稳定 `NodeId` 和该容器 `InlineDocument.source` 中的源码偏移。inline CST 节点身份可以辅助命中测试和交互状态协调，但不得成为 Selection 的第二套权威坐标。

块内编辑命令应产生 `TextEdit` 并只重解析受影响内容节点的 inline CST；块级命令应表达为节点的拆分、合并、插入、删除、移动、提升和包裹等统一树操作。两者不得混用全文 Markdown 字符串作为编辑中介。

操作完成后必须：

* 维护并验证统一块树及块内 lossless CST 不变量；
* 修复并恢复 Selection；
* 更新派生渲染和布局状态。
...
* 序列化 Markdown；
* Undo/Redo；
* 未编辑内容的 parse–serialize 逐字符无损，以及重新解析后的结构一致性。
...
* 段落内部删除只编辑对应内容节点的 `InlineDocument.source`、局部重解析 CST，并保留未受影响的节点身份；
