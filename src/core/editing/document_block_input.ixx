...
inline std::optional<document_edit_detail::RecordedBlockEdit> replace_paragraph_with_container(
    ...
    if (match.kind == MarkerMatch::Kind::Heading) {
        ...
        source_block.ensure_special().level = match.heading_level;
        source_block.ensure_special().opening_marker = exact_marker;
        source_block.ensure_special().closing_marker.clear();
        source_block.ensure_special().slug.clear();
    ...
    if (match.kind == MarkerMatch::Kind::Quote) {
        ...
        quote.ensure_special().opening_marker = exact_marker;
    ...
    list.ensure_special().list_ordered = match.kind == MarkerMatch::Kind::OrderedList;
    list.ensure_special().list_start = list.special().list_ordered ? match.list_start : 1;
    list.ensure_special().list_delimiter = match.list_delimiter;
    BlockNode item;
    item.id = allocator.allocate();
    item.kind = match.kind == MarkerMatch::Kind::TaskList ? BlockKind::TaskListItem : BlockKind::ListItem;
    item.ensure_special().marker = exact_marker;
    item.ensure_special().checked = match.checked;
...
inline std::optional<document_edit_detail::RecordedBlockEdit> open_raw_block(
    ...
    replacement.ensure_special().math_delim = opening->math_delimiter;
...
inline std::optional<document_edit_detail::RecordedBlockEdit> remove_heading_prefix(
    ...
    block->ensure_special().level = 0;
    block->ensure_special().slug.clear();
    block->ensure_special().opening_marker.clear();
...
inline std::optional<document_edit_detail::RecordedBlockEdit> remove_callout_prefix(
    ...
    callout->ensure_special().callout_kind.clear();
    callout->ensure_special().opening_marker.clear();
    callout->ensure_special().closing_marker.clear();
