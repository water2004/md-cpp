        ...
        folia::SearchOptions options)
    {
        ...
        if (core_->sourceEditor)
        {
            ...
            for (auto const& match : result.matches)
            {
                ...
                {
                    auto position = core_->sourceEditor->position_from_source_offset(match.range.start);
                    target.spans.push_back({position.container_id, {position.source_offset, position.source_offset}});
                }
