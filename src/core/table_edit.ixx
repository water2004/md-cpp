...
inline std::optional<TableRowSource> parse_table_source_row(std::u32string_view text, std::size_t line_start, std::size_t line_end) {
    ...
    for (std::size_t index = 0; index <= line.size(); ++index) {
        if (index == line.size() || line[index] == U'|') {
            if (!(index == 0 || (index == line.size() && segment_start == index))) {
...
inline std::u32string format_table_matrix(std::vector<std::vector<std::u32string>> const& matrix, bool trailing_newline) {
    ...
    for (std::size_t column = 0; column < column_count; ++column) {
        out.push_back(U' ');
        for (std::size_t index = 0; index < widths[column]; ++index) out.push_back(U'-');
        out.push_back(U' ');
        out.push_back(U'|');
    }
    out.push_back(U'\n');
...
inline std::optional<Transaction> table_edit_transaction(TableEditKind kind, std::u32string_view text, Selection selection, std::uint64_t revision) {
    ...
    auto replacement = changed ? format_table_matrix(matrix, table->trailing_newline) : std::u32string(text.substr(table->range.start.v, table->range.len()));
    auto caret = table->range.start.v + formatted_cell_offset(matrix, target_row, target_column);
    if (!changed) {
